...
def format_pairs(pairs):
    ...
    lines = []
    for cp, val in pairs:
        lines.append(f"  #({hex(cp)}, {json.dumps(val)}),")
    return "\n".join(lines)
...
def render_pages_module(pairs, module, name, page_size=256, word_bits=64):
    ...
    import json
    # Use json.dumps to ensure proper escaping for Gleam string literals
    pool_strings_str = ',\n  '.join(json.dumps(s) for s in pool_strings)
...
def main():
    ...
    if args.mode == 'pages':
        ...
        # Emit some stats to help compare encodings
        masks, offsets, pool_strings, flat_indices = build_pages(pairs, page_size=256, word_bits=args.word_bits)
        words_per_page = (256 + args.word_bits - 1) // args.word_bits
        total_words = sum(len(w) for w in masks.values())
        print(f"pages={len(masks)} pool_size={len(pool_strings)} flat_indices={len(flat_indices)} words_per_page={words_per_page} total_words={total_words}")
    else:
