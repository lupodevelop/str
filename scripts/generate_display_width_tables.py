#!/usr/bin/env python3
"""
Generator for Unicode display width tables with binary search.

Downloads EastAsianWidth.txt and UnicodeData.txt from Unicode 15.1.0 and produces
`src/str/internal_display_width.gleam` with O(log n) binary search lookup.
...
def generate_binary_search_code(ranges, indent, default_result, ambiguous_var=None):
    """
    Generate Gleam code for binary search over sorted ranges.
    Each range is (start, end, width) for width ranges or (start, end) for combining.
    Returns list of code lines.
    """
    if not ranges:
        return [f"{indent}{default_result}"]
    
    if len(ranges) == 1:
        r = ranges[0]
        if len(r) == 3:
            start, end, width = r
            if width == 3 and ambiguous_var:
                result = ambiguous_var
            else:
                result = str(width)
        else:
            start, end = r
        ...
        lines = []
        lines.append(f"{indent}case cp >= {start} && cp <= {end} {{")
        lines.append(f"{indent}  True -> {result}")
        lines.append(f"{indent}  False -> {default_result}")
    ...
    # Use the start of the first right range as pivot
    pivot = right_ranges[0][0]
    
    lines = []
    lines.append(f"{indent}case cp < {pivot} {{")
    lines.append(f"{indent}  True -> {{")
    lines.extend(generate_binary_search_code(left_ranges, indent + "    ", default_result, ambiguous_var))
    lines.append(f"{indent}  }}")
    lines.append(f"{indent}  False -> {{")
    lines.extend(generate_binary_search_code(right_ranges, indent + "    ", default_result, ambiguous_var))
    lines.append(f"{indent}  }}")
    lines.append(f"{indent}}}")
    return lines
...
def generate_gleam(out_path: Path, eaw_ranges, combining_set):
    """Generate Gleam module with binary search lookup."""
    ...
    combining_ranges.sort(key=lambda x: x[0])
    ...
    lines.append('//// Lookup: O(log n) binary search')
    lines.append('')
    
    # Generate is_combining function with binary search
    lines.append('/// Check if codepoint is a combining mark (width 0)')
    lines.append('/// O(log n) binary search over sorted ranges')
    lines.append('fn is_combining(cp: Int) -> Bool {')
    lines.extend(generate_binary_search_code(combining_ranges, "  ", "False"))
    lines.append('}')
    lines.append('')
    
    # Generate width lookup function with binary search
    lines.append('/// Find width for codepoint: 2 = wide, 3 = ambiguous, 0 = not found')
    lines.append('/// O(log n) binary search over sorted ranges')
    lines.append('fn find_special_width(cp: Int) -> Int {')
    lines.extend(generate_binary_search_code(special_ranges, "  ", "0"))
    lines.append('}')
    lines.append('')
    ...
    # Calculate tree depth for O(log n) verification
    import math
    combining_depth = math.ceil(math.log2(max(1, len(combining_ranges))))
    width_depth = math.ceil(math.log2(max(1, len(special_ranges))))
    
    print(f'Wrote {out_path}')
    print(f'  - {len(special_ranges)} width ranges (max depth: {width_depth})')
    print(f'  - {len(combining_ranges)} combining ranges (max depth: {combining_depth})')
