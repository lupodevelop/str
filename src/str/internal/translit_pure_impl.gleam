// Target-specific transliteration implementation
...
@target(javascript)
import str/internal/generated_translit_pages_js as pages_gen_js

/// Erlang: use 64-bit optimized page tables
@target(erlang)
pub fn transliterate_impl(s: String) -> String {
...
pub fn transliterate_impl(s: String) -> String {
  let gs = string.to_graphemes(s)
  list.fold(gs, [], fn(acc, g) {
    case pages_gen_js.translit_pages_js_lookup_by_grapheme(g) {
      Ok(v) -> [v, ..acc]
      Error(_) -> [g, ..acc]
    }
  })
